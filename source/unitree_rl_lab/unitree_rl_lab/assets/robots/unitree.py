...
##
# Skating variant: G1-29DOF with skate physics properties.
#
# The skate attachment is realised purely through physics material overrides
# at the ankle roll links, combined with a low-friction rink surface.
# When init_state.pos.z is raised by ~0.04 m (wheel radius) to account for
# the wheel height above the ground.
#
# Ankle actuator stiffness is reduced relative to the locomotion config to
# allow compliant ankle motion during gliding and edge-control.
##

UNITREE_G1_29DOF_SKATE_CFG = UnitreeArticulationCfg(
    spawn=UnitreeUsdFileCfg(
        usd_path=f"{UNITREE_MODEL_DIR}/G1/29dof/usd/g1_29dof_rev_1_0/g1_29dof_rev_1_0.usd",
    ),
    init_state=ArticulationCfg.InitialStateCfg(
        # Root height accounts for wheel geometry:
        #   wheel_bottom_below_ankle = 0.088 (center) + 0.038 (radius) = 0.126 m
        #   sphere_foot_bottom_below_ankle ≈ 0.050 m
        #   extra raise = 0.126 - 0.050 = 0.076 m over the 0.80 m G1 baseline → 0.876 m
        pos=(0.0, 0.0, 0.876),
        joint_pos={
    ...
    actuators={
        ...
        # Ankle joints: reduced stiffness for compliant skate edge control
        "N5020-16-ankle": ImplicitActuatorCfg(
            joint_names_expr=[".*_ankle_pitch_joint", ".*_ankle_roll_joint"],
            effort_limit_sim=35,
            velocity_limit_sim=30,
            stiffness=25.0,   # softer than locomotion (40 → 25) for edge compliance
            damping=3.0,
            armature=0.01,
        ),
        "W4010-25": ImplicitActuatorCfg(
