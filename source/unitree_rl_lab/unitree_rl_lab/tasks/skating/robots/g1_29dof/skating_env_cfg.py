...
Phase 1 — G1SkatingGlideEnvCfg (defined here):
  Both feet on skates, no push-off. Train balance and passive gliding.
  Velocity command is small (0.5–1.5 m/s), no angular velocity.

Phase 2+ — G1SkatingEurekaEnvCfg (defined in eureka_env_cfg.py):
  Full skating behavior (push-off, speed, turning) with reward function
  proposed iteratively by NVIDIA Eureka (LLM-driven reward search).
  See scripts/skating/eureka_phase2.py for the Eureka runner.

Shared scene, observation, action, and termination configs live here
and are imported by eureka_env_cfg.py.
"""
